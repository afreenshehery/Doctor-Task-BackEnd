...
const Doctor = require('../models/Doctor');
const tree = require('../models/treeView')
const GetTree = require('../models/getTree')
const Recursive = require('../models/recursive');
const { JSON } = require('sequelize');
const Cuntries = require('../models/cuntries')
...
let edit_patient = async (req, id, HospitalId,) => {
    console.log("manager reached");
    ...
    console.log(edit_patient.hospitalId);
...
let treeView = async (body) => {
    console.log("manager reached");
    console.log(body.course);
    let storeData = body.course
    let allPlayers = "" + storeData.join(",") + "";
    console.log(allPlayers);
    let data = {
        course: allPlayers
    }

    allData = await tree.create(data);
    console.log(allData.course);
    return { data: data.course }

}

let getTreeView = async () => {
    console.log("here");
    let data = await tree.findOne({ raw: true })

    console.log(data.course);
    return { data: data.course, id: data.id }
}

let putTreeView = async (req, body) => {
    console.log("yjhgh", body);
    let data = body.course
    let allPlayers = "" + data.join(",") + "";
    let findData = {
        id: body.id
    ...
    console.log(allPlayers);
    let updatedata = await tree.update(data1, { where: findData, row: true })
    console.log(updatedata);
    return updatedata
}
// ------------------------------
let sendRecursive = async (body) => {
    console.log("manager reached");
    console.log(body);
    // dataData = body
    let storeData = {
    ...
    allData = await Recursive.create(storeData);

    return { data: data }
}
let getRecursive = async () => {
    console.log("here");
    let data = await Recursive.findAll({ raw: true })

    console.log(data);
    return { data: data }
}

let cuntryDerails = async (body) => {
    console.log("manager reached");
    console.log(body);
    ...
    allData = await Cuntries.create(data);
    // console.log(allData.course);
    data = await Cuntries.findAll({ row: true })
    // let allPlayers = "" + data.join(",") + "";
    console.log(allData);
...
let getCuntry = async () => {
    console.log("here");
    let data = await Cuntries.findAll({ raw: true })
...
module.exports = { Admin_Register, getCuntry, cuntryDerails, sendRecursive, getRecursive, getTreeView, putTreeView, treeView, Admin_login, Add_Petient, Add_staff, destroy_patient, edit_patient, destroy_staff, edit_staff, get_staff, get_patient, get_staff_update, get_patient_update, getAdmin, getRegisterInfo, editAdmin };